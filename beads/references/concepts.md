...
- **SQLite backend**: `.beads/*.db` is the primary store; `bd --no-daemon sync` exports to `.beads/issues.jsonl` for git sync.
...
- **Dolt**: `bd --no-daemon init --backend dolt` stores data in `.beads/dolt/` with versioned history.
